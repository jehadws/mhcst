      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input id="venue" value={data.venue} onChange={(e) => setData('venue', e.target.value)} placeholder="مثال: القاعة الرئيسية - طرابلس" />
          </div>
                ...
                return (
                  <div key={inst.id} className="flex items-center gap-2">
                    <Checkbox
                      id={`inst-${inst.id}`}
                      checked={selected}
                      onCheckedChange={(checked) => handleInstructorToggle(inst.id, !!checked)}
                    />
          ...
          <div>
            <ImageUploader value={data.cover_image} onChange={(path) => setData('cover_image', path)} folder="courses" label="صورة الغلاف" />
            {errors.cover_image && <p className="text-sm text-red-500 mt-1">{errors.cover_image}</p>}
          </div>

          <div className="flex justify-end gap-2 pt-4">
            <Button type="button" variant="outline" onClick={() => router.get(route('dashboard.courses.list'))}>إلغاء</Button>
